...
Arquivos/pastas: `client/Assets/Scripts/Network/Protocol.cs`, `WorldConnection.cs`, `NetworkSession.cs`, novo `NetworkChatController.cs` (log de mensagens + campo de texto no canto inferior esquerdo, acima da dica de controles fixa do `DesktopControlsBootstrap.cs`). Crônicas/mural ficou fora — a migração `004_world_events.ts` documenta que a UI do mural é "fora de escopo até o Corte 4", sem endpoint de leitura ainda. Nenhuma mudança no servidor (já estava pronto em produção).
Descrição/Resultado: `NetworkChatController.InputFocused` (estático) é checado agora em `NetworkEconomyController.cs` (M minerar, E contexto), `NetworkCombatController.cs` (F atacar, Tab alvo, clique de seleção) e `NetworkMapController.cs` (N mapa) — sem isso, digitar no chat dispararia essas ações, já que o `Input.GetKeyDown` legado do Unity não sabe que um campo de UI está focado. **Lacuna que não dá pra fechar sem tocar `PrototypePlayerController.cs`** (fora do meu escopo agora, Codex "EM ANDAMENTO" lá): WASD ainda move o personagem enquanto o jogador digita no chat. Fica pra quando alguém construir o estado de foco compartilhado descrito na seção "Entrada" do plano do Codex. Compilação batchmode real validada (0 `error CS`). Build WebGL novo (114.792.905 bytes) republicado no serviço Railway `webgl` — deploy `38ee892a-681f-4028-98ca-7519ae101d80`, status `SUCCESS`.
...
Resultado: nova constante `AttackRangeUnits = 2.25f` espelhando `PLAYER_ATTACK_RANGE_UNITS` do servidor; botão ATACAR só fica clicável dentro do alcance real; texto de alvo mostra "· fora de alcance" quando selecionado mas longe demais; apertar F/clicar fora de alcance agora mostra mensagem local na hora em vez de mandar uma requisição que o servidor sempre recusaria. **Não resolvido nesta rodada**: o descompasso de intervalo entre ataques (debounce local de 0,2s vs. intervalo real 0,7-1,5s por agilidade) — exigiria expor o atributo de agilidade neste controller, que hoje não tem acesso a atributos; fica pra uma próxima passada. Compilação batchmode real validada (0 `error CS`). Build WebGL novo (114.776.152 bytes, já inclui o `MovementDiagnostics.cs` do Codex presente na árvore) republicado no serviço Railway `webgl` — deploy `59580360-4515-4ab4-a53a-5cbc48c49e7c`, status `SUCCESS`.
...
Nota: log estava livre no momento (reserva documental do Codex acima já tinha concluído, nenhuma outra ativa) — usuário pediu explicitamente pra eu incluir a tela em `NetworkUI.cs` também, mesmo sabendo que o Codex tinha acabado de propor um plano de UI que pode cobrir a mesma tela; se colidir, ajustar depois. Aparência visual de fato (raça/gênero no modelo 3D) fica fora do escopo — isso é arte/Codex; aqui foi só o fluxo de escolha + dado salvo/devolvido pelo servidor.
Resultado: sem migração (a coluna `appearance_json` já existia, default `{}`, nunca usada); fallback humano/masculino tanto pra requisições sem esses campos quanto pra personagens antigos com `appearance_json` vazio. `npx tsc --noEmit` limpo em contracts/server; suíte completa 15 arquivos, 125 testes passando (4 novos). Compilação C# do cliente validada em batchmode (0 `error CS`). **Validado ponta a ponta em produção**: registrei uma conta descartável, criei personagem com `race: "elfo", gender: "feminino"` via `POST /characters` real contra `server-production-266d.up.railway.app` — resposta confirmou os dois campos salvos e devolvidos corretamente. Servidor republicado (deploy `7266d636-e7c3-48e6-a445-a4861215d558`, commit `b23ca2d`, `SUCCESS`) e build WebGL novo (114.748.789 bytes) republicado no serviço `webgl` (deploy `d2a75aae-ad02-4b1c-8279-50bfe8879e1b`, `SUCCESS`).
