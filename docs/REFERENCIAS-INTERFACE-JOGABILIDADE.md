...
### Atualização de status — itens da tabela "Diagnóstico atual" que fechei (10/09/2026, tarde)

Pedido do usuário: fechar nesta sessão o que estivesse pendente neste documento e fosse meu (servidor/protocolo/não-visual). Não altero as linhas da tabela do Codex acima — só registro aqui o que mudou de estado, pra quem ler saber o que ainda é real e o que já foi resolvido.

- **"Disponibilidade de ataque não acompanha a regra real" — RESOLVIDO.** `NetworkCombatController.cs` agora usa `AttackRangeUnits = 2.25f` (espelha `PLAYER_ATTACK_RANGE_UNITS` do servidor) pra habilitar o botão ATACAR e pra recusar localmente um ataque fora de alcance com mensagem imediata, em vez de mandar uma requisição que o servidor sempre recusaria. **Ainda não resolvido**: o descompasso de *intervalo* entre ataques (debounce local de 0,2s vs. intervalo real por agilidade) — exigiria expor o atributo de agilidade nesse controller, que hoje não tem acesso a atributos.
- **"Chat/crônicas ainda não têm ligação na camada de cliente auditada" — chat RESOLVIDO, crônicas continua fora de escopo.** `WorldConnection`/`Protocol`/`NetworkSession` agora enviam e recebem `chat.send`/`chat.message` de verdade, com um painel de log + campo de texto (`NetworkChatController.cs`, canto inferior esquerdo). Crônicas/mural não têm endpoint de leitura ainda — a migração `004_world_events.ts` já documentava isso como "fora de escopo até o Corte 4"; não é uma pendência esquecida, é uma decisão de escopo anterior a este plano.
- **"Recuperação de senha ainda não funciona" — servidor RESOLVIDO, falta a tela.** `POST /auth/password-reset/request` e `/confirm` implementados de verdade: token de uso único com hash, expira em 30 minutos, resposta sempre neutra (nunca revela se o e-mail existe), e trocar a senha revoga todas as sessões ativas da conta. Testado de ponta a ponta contra Postgres real (local e produção). **A única dependência que continua de verdade pendente**: não existe provedor de e-mail configurado no projeto (nenhuma lib de SMTP/Resend/SendGrid no `package.json`, nenhuma variável de ambiente) — o endpoint de request devolve o token bruto como `devToken` fora de produção (só pra testar o fluxo), mas em produção a resposta nunca inclui o token, então o link não chega em lugar nenhum até alguém escolher e configurar um provedor real. A tela de "esqueci a senha" em si (formulário de e-mail → aviso → nova senha) não foi construída — fica pro Codex, que está iterando ativamente na tela de login. `ApiClient.RequestPasswordResetAsync`/`ConfirmPasswordResetAsync` já existem prontos (mesmo padrão dos outros métodos HTTP) pra essa tela só chamar quando for construída.
- **"Lembrar sessão e saída precisam ser concluídos" — parcialmente RESOLVIDO.** Achei um bug real: "SAIR DO ONLINE" nunca chamava `/auth/logout` — o refresh token continuava válido no servidor por até 30 dias (`JWT_REFRESH_TTL_DAYS`) mesmo depois de "sair". Agora `ReturnToLocal()` chama `ApiClient.LogoutAsync` (best-effort, não bloqueia a saída se a rede falhar) e limpa o token salvo em `PlayerPrefs`. **Ressalva importante**: "SAIR DO ONLINE" hoje volta pro modo de exploração local (não pra tela de login) — o próprio plano pede pra "eliminar transições de produção para exploração local" (§2). Revogar o token aqui é o comportamento mais seguro e correto independente de como isso for redesenhado, mas o redesenho de verdade da tela de entrada/saída (escolha explícita de "lembrar e-mail" vs. "manter conectado", fluxo de logout que volta à tela de login) continua sendo trabalho do Codex na etapa 2.

**O que eu não tentei fechar** (fora do meu alcance ou não fechável sem decisão externa nesta rodada): tremor de movimento/câmera, animação de combate, posicionamento dos NPCs no mundo, linha de base de desempenho, CI cobrindo o cliente (precisaria de licença/segredos do Unity em CI, uma decisão de infraestrutura que não é minha pra tomar sozinho) — todos continuam exatamente como o Codex diagnosticou.
