...
- **Redistribuição gratuita** (GDD §6/§13: "permite redistribuição gratuita durante o teste, falando com a instrutora"): `attribute.respec` (sem payload) → `attributes.snapshot` — devolve todos os pontos já alocados (a diferença de cada atributo pro valor inicial, 5) pra `unspentPoints` e reseta os 4 atributos. Sempre bem-sucedido, mesmo sem nada pra redistribuir (é um no-op inofensivo, não um erro). Não há limite de vezes, como o GDD pede. Testado (`server/tests/unit/attributes.service.test.ts`) e validado ponta a ponta em 10/09/2026: personagem com atributos desequilibrados (8/5/7/6, 2 pontos livres) volta pra 5/5/5/5 com 8 pontos livres (6 devolvidos + 2 que já estavam livres) e `maxHp` recalculado; redistribuir de novo sem nada alocado não muda nada.
